...
void Null_Fire(edict_t *ent)
{
	...
	//	if (!((int)dmflags->value & DF_INFINITE_AMMO ) )
	//		ent->client->pers.inventory[ent->client->ammo_index]--; // comment these out to prevent the Minus NULL Ammobug
}
...
void CTFGrappleDrawCable(edict_t *self)
{
	...
	gi.WriteByte(svc_temp_entity);
	gi.WriteByte(TE_GRAPPLE_CABLE); //te not working. other te's crash game
...
void CTFGrapplePull(edict_t *self)
{
	...
	if ((strcmp(self->owner->client->pers.weapon->classname,"weapon_grapple") == 0) &&!self->owner->client->newweapon && (self->owner->client->weaponstate != WEAPON_FIRING) &&
